...
mat4 camera_interpolated_view(struct camera* cam, float interpolaton)
{
    ...
    mat4_to_array(cam->prev_view_mat, (float*)prev);
    mat4_to_array(cam->view_mat, (float*)new);
    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
            out[i][j] = prev[i][j] + (new[i][j] - prev[i][j]) * interpolaton;
    mat4 mat_out;
    memcpy(&mat_out, out, sizeof(mat4));
    return mat_out;
}
