                                   ...
                                   const size_t depth)
{
    ...
    vector_init(&self->nodes, sizeof(ivec3));
    self->used = 0;
    self->width = width;
    self->height = height;
    self->depth = depth;
                      ...
                      const size_t height)
{
    ...
    node = (ivec3*)(vector_at(&self->nodes, index));
    x = node->x;
    y = node->y;
    width_left = width;
    i = index;
    ...
    y = node->y;
    while (width_left > 0) {
        node = (ivec3*)(vector_at(&self->nodes, i));
        if (node->y > y)
            y = node->y;
...
void texture_atlas_merge(texture_atlas_t* self)
{
    ...
    for (i = 0; i < self->nodes.size - 1; ++i) {
        node = (ivec3*)(vector_at(&self->nodes, i));
        next = (ivec3*)(vector_at(&self->nodes, i + 1));
        if (node->y == next->y) {
            node->z += next->z;
            vector_remove(&self->nodes, i + 1);
            --i;
        }
    }
}
                               ...
                               const size_t height)
{
    ...
    for (i = best_index + 1; i < self->nodes.size; ++i) {
        node = (ivec3*)vector_at(&self->nodes, i);
        prev = (ivec3*)vector_at(&self->nodes, i - 1);
